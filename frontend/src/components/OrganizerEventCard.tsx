...
export default function OrganizerEventCard({
  ...
  return (
    <div className="group flex flex-col gap-3 rounded-card border border-border bg-white p-4 shadow-soft transition-all duration-200 hover:border-secondary hover:shadow-soft-lg sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="flex gap-2 sm:flex-col sm:items-end">
        <button
          ...
          onClick={onEdit}
          disabled={disabled}
          className="btn-secondary flex-1 text-xs sm:w-28"
        >
        ...
        <button
          ...
          onClick={onDelete}
          disabled={disabled}
          className="inline-flex flex-1 items-center justify-center rounded-button border-2 border-red-300 bg-red-50 px-4 py-2.5 font-body text-xs font-semibold text-red-700 shadow-soft transition-all duration-200 hover:bg-red-100 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-50 sm:w-28"
        >
          <svg className="mr-1 h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
